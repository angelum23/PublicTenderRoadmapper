...
public class ExamController(IExamApplication aplic) : BaseController<Exams>(aplic)
{
    ...
    public async Task<IActionResult> AddWithNotice([FromBody] ExamWithNoticeDto entity) => await Controller(aplic.Add(entity));
    
    
    [HttpPost("AddWithPrompt")]
    public async Task<IActionResult> AddWithPrompt([FromBody] ExamWithPromptDto entity) => await Controller(aplic.Add(entity));
